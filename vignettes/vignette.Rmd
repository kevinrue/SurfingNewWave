...
to each single cell and the dispersion parameter can be equal to all observation or related
to each single gene.
...
We can reduce even more the time consumption using some mini batch approaches on the different steps of the optimization.
...
The are some differences with the matrix implementation:
 - there is a small aproximation in the initialization
...
run this code becouse it would be long.

```{r}
# res4 <- newWave(tenx, K=10, children=2,n_gene_par = 100, n_cell_par = 100, commondispersion = FALSE)
```

The following figures explain the performance of NewWave in this dataset with differents approach:
